...
public class MP3_Gui extends  JPanel implements ActionListener{
		...
		MP3_Gui(int index) {
			...
			manager = new AudioManager(index);
			manager.getAllSongs(); //get all the songs to songList
			currSong = manager.songList.get(manager.index);
			
			
			Icon albumIcon = new ImageIcon(currSong.songAlbumCover);
			albumCover = new JLabel(albumIcon);
			albumCover.setSize(250,250);
			albumCover.setLocation(55,200);
			add(albumCover);
			
			songLabel = new JLabel(currSong.songTitle, SwingConstants.CENTER);
			//songLabel.setText(currSong.songTitle);
			songLabel.setSize(100,100);
			songLabel.setLocation(120,80);
			add(songLabel);
			
			artistName = new JLabel();
			artistName.setText(currSong.artist);
			artistName.setSize(80,80);
			artistName.setLocation(160,145);
			add(artistName);
